...
The top level contains summary files and one sub-directory per tolerance value:

```
RESULTS_DIR/
└── zz1d/                            # one directory per experiment (here example is ZZ for 1D transition layer)
    ├── convergence.csv              # per-tolerance convergence data (DOFs, η, errors, rates)
    ├── final_metrics_summary.txt    # aggregated summary across all tolerances
    ├── run_info.txt                 # run configuration (problem, estimator, parameters)
    ├── run_20260710_112634.log      # timestamped stdout/stderr log
    ├── tol_0.5/                     # ─┐
    ├── tol_1.0/                     #  │  one sub-directory per tolerance value
    ├── tol_2.0/                     #  │
    └── ...                          # ─┘
```

Each `tol_<value>/` sub-directory holds all per-iteration files for that tolerance run:

```
tol_0.5/
├── mmg_output.txt               # raw MMG3D stdout/stderr (remeshing log)
├── meshes/                      # MEDIT .mesh files and anisotropic metric .sol files for all iterations
├── sols/                        # FE solution snapshots (MEDIT .sol format)
├── vtk/                         # ParaView-ready VTK files
└── compute_errs/
    └── computed_quantities.bp   # ADIOS2 binary pack — error norms, DOF counts, η values
```

## Experiments on fixed meshes

In the `tests/` directory we test the estimators on fixed meshes generated by Fenicsx. This is done in comparison to the Delaunay meshes gnerated by MMG.

| Script | Mesh Type | Space |
| --- | --- | --- |
| `3D_Poisson_error.py` | Isotropic | ZZ (P1)
| `3D_Poisson_aniso_error.py` | Anisotropic | ZZ (P1)
| `3dNZ.py` | Isotropic | NZ (P2)
| `3dNZ_aniso.py` | Anisotropic | NZ (P2)

The `testNZ.py` checks the polynomial-preserving recovery property of the NZ method on a solution with exact polynomial. All these files can be run with:
```bash
